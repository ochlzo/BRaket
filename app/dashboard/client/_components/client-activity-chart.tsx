...
function buildClientSeries(bookings: BookingListItem[]) {
...
export function ClientActivityChart({ bookings }: ClientActivityChartProps) {
  const [rangeDays, setRangeDays] = useState<RangeDays>(90);
  const fullSeries = useMemo(() => buildClientSeries(bookings), [bookings]);
    ...
    <Card className="border-[color:var(--line-strong)] bg-[color:var(--surface)] shadow-[var(--shadow-surface-soft)]">
      <CardHeader className="gap-3 md:grid-cols-[1fr_auto]">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <CardTitle className="tracking-normal">Project activity</CardTitle>
            <Badge className="bg-[color:var(--tone-sky-soft)] text-[color:var(--tone-sky-deep)]">
              Live data
            </Badge>
          </div>
          <CardDescription>
            Requests sent and completed projects for this client account.
          </CardDescription>
        </div>
      ...
      <CardContent>
        <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="text-xs font-medium text-[color:var(--ink-muted)]">
              Requests sent in {selectedLabel}
            </p>
          </div>
        ...
        <div className="overflow-hidden rounded-lg border border-[color:var(--line)] bg-[color:var(--surface-alt)]/40">
          <svg
            aria-label="Client project activity chart"
          ...
          >
            <defs>
              <linearGradient id="client-requests-fill" x1="0" x2="0" y1="0" y2="1">
                <stop offset="0%" stopColor="var(--brand-orange)" stopOpacity="0.42" />
                <stop offset="100%" stopColor="var(--brand-orange)" stopOpacity="0.02" />
              </linearGradient>
              <linearGradient id="client-completed-fill" x1="0" x2="0" y1="0" y2="1">
            ...
            <path
              ...
              fill="url(#client-requests-fill)"
            />
            <path
              d={makeAreaPath(completedValues, width, height, maxValue)}
              fill="url(#client-completed-fill)"
            />
            <path
