...
export default async function TalentProfilePage({ params }: Props) {
  ...
  const talentReviews = getReviewsByTalent(talent.id);
              ...
              {talentServices.length === 0 ? (
                ...
                <div className="grid gap-5">
                  {talentServices.map((service) => (
                    <div
                      key={service.id}
                      className="group flex flex-col sm:flex-row gap-6 p-6 rounded-2xl border border-[color:var(--line-strong)] bg-white transition-all hover:shadow-lg"
                    >
                      ...
                      <div className="flex flex-col sm:items-end justify-between gap-4 mt-4 sm:mt-0 pt-4 sm:pt-0 border-t sm:border-0 border-[color:var(--line)]">
